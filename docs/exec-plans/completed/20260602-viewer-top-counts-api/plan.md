...
4. ✅ Step 3 に依存して、`src/server/packages/SiteStats/Application/Viewer/Query/{SiteStats.php,SiteStatsQueryServiceInterface.php}` と `src/server/packages/SiteStats/Infrastructures/Viewer/SiteStatsQueryService.php` を追加する。`SiteStatsQueryService` は `Song::query()->where('is_display', true)->count()` で公開楽曲数を取得し、既存 `list()` / `get()` の `is_display = true` 条件と同じ公開判定を使う
5. ✅ Step 4 に依存して、`src/server/packages/SiteStats/Application/Viewer/UseCase/Get/{GetUseCase.php,GetOutputData.php}` を追加する。`GetUseCase` は `SiteStatsQueryServiceInterface::get()` を呼び、`Result<GetOutputData, UseCaseError>` の `Ok` として `songCount` を返す
...
- 2026-06-02: 集計 API の use case、query、route map は `SiteStats` package に置く。初回実装の集計対象は楽曲だけだが、将来 media / release を同じ API で集約するため、`Song` package に site stats 用の orchestration や件数取得メソッドを持たせない
