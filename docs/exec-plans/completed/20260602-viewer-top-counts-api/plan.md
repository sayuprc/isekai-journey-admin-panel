...
3. ✅ `src/server/packages/Support/Route/ViewerSiteStatsRouteMap.php` を新設し、`case Get = 'viewer.site-stats.get';` を定義する。`src/server/routes/viewer.php` に `GetSiteStatsController` と route map の import を追加し、`Route::get('/site-stats', [GetSiteStatsController::class, 'handle'])->name(ViewerSiteStatsRouteMap::Get);` を `v1` 配下に登録する。
...
5. ✅ Step 4 に依存して、`src/server/packages/Song/Application/Viewer/UseCase/Count/{CountUseCase.php,CountOutputData.php}` を追加する。`CountUseCase` は `SongQueryServiceInterface::countDisplayable()` を呼び、`Result<CountOutputData, UseCaseError>` の `Ok` として `songCount` を返す。
6. ✅ Step 5 に依存して、`src/server/app/Http/Controllers/Api/Viewer/V1/SiteStats/GetSiteStatsController.php` と `src/server/app/Http/Presenters/Api/Viewer/V1/SiteStats/GetPresenter.php` を追加する。controller は入力なしで `CountUseCase` を実行し、presenter は成功時に `['songCount' => $outputData->songCount]` を 200 で返し、失敗時は既存 presenter と同じ `ResolvesUseCaseError` を使う。
...
- 2026-06-02: 公開楽曲数の取得は初回実装では `Song\Application\Viewer` / `Song\Infrastructures\Viewer` に置く。現時点の集計対象は楽曲だけで、既存の公開判定も Song Viewer query に集約されているため。将来 media / release を追加する時点で、`SiteStats` 用の複数ドメイン集約 use case へ移す余地を残す。
...
- Server static checks: `mise run api:phpstan packages/Song app/Http/Controllers/Api/Viewer/V1/SiteStats app/Http/Presenters/Api/Viewer/V1/SiteStats tests/Feature/Api/Viewer/V1/SiteStats` と `mise run api:ecs packages/Song app/Http/Controllers/Api/Viewer/V1/SiteStats app/Http/Presenters/Api/Viewer/V1/SiteStats tests/Feature/Api/Viewer/V1/SiteStats` を実行する。
