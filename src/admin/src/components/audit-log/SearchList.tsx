...
const ACTION_OPTIONS = ['create', 'update', 'delete', 'register', 'login', 'refresh', 'recovery_code_issue'] as const satisfies readonly Action[];
    ...
    <>
      <form onSubmit={handleSearch} class="mb-4 flex flex-wrap items-end gap-4">
        <fieldset class="fieldset">
          <label class="fieldset-label" for="from">期間 From</label>
          <input
            type="datetime-local"
            id="from"
            name="from"
        ...
        <fieldset class="fieldset">
          <label class="fieldset-label" for="to">期間 To</label>
          <input
            type="datetime-local"
            id="to"
            name="to"
        ...
        <fieldset class="fieldset">
          <label class="fieldset-label" for="action">操作</label>
          <select
            id="action"
            name="action"
          ...
          >
            <option value="" selected={inputAction() === undefined}>すべて</option>
            <For each={ACTION_OPTIONS}>
              {a => (
                <option value={a} selected={inputAction() === a}>{ACTION_LABEL[a]}</option>
              )}
            </For>
          </select>
        </fieldset>
        <fieldset class="fieldset">
          <label class="fieldset-label" for="target_type">対象種別</label>
          <select
            id="target_type"
            name="target_type"
            class="select select-bordered select-sm"
            onChange={e => setInputTargetType(e.currentTarget.value === '' ? undefined : (e.currentTarget.value as TargetType))}
          >
            <option value="" selected={inputTargetType() === undefined}>すべて</option>
            <For each={TARGET_TYPE_OPTIONS}>
              {t => (
                <option value={t} selected={inputTargetType() === t}>{TARGET_TYPE_LABEL[t]}</option>
              )}
            </For>
          </select>
        </fieldset>
        <fieldset class="fieldset">
          <label class="fieldset-label" for="target_id">対象ID</label>
          <input
            type="text"
            id="target_id"
            name="target_id"
        ...
        <fieldset class="fieldset">
          <label class="fieldset-label" for="admin_user_name">実行者名</label>
          <input
            type="text"
            id="admin_user_name"
        ...
        <fieldset class="fieldset">
          <label class="fieldset-label" for="per_page">表示件数</label>
          <select
            id="per_page"
            name="per_page"
            class="select select-bordered select-sm"
          ...
          >
            <For each={PER_PAGE_OPTIONS}>
              {n => <option value={n} selected={inputPerPage() === n}>{n}件</option>}
            </For>
          </select>
        </fieldset>
        <button type="submit" class="btn btn-primary btn-sm mb-1">適用</button>
                  ...
                  <For each={result().auditLogs}>
                    {log => (
                      <tr class="hover:bg-primary/30 focus-within:bg-primary/30 transition-colors">
                        ...
                        <td>
                          <a href={`/audit-logs/${log.auditLogId}?back=${encodeURIComponent(window.location.search)}`} class="btn btn-ghost btn-xs">
                            詳細
                          </a>
                        </td>
                      </tr>
