...
readonly class TrackEntries extends ImmutableCollection
{
    ...
    public static function fromArray(array $items): Result
    {
        ...
        foreach ($items as $item) {
            ...
            )->map(fn (array $values): TrackEntry => new TrackEntry(...$values));
            ...
            if (isset($seenSongIds[$entry->songId->value])) {
                return new Err(new DomainValidationError([
                    'trackEntries' => ['同じ楽曲を複数指定することはできません。'],
                ]));
            }

            if (isset($seenTrackNos[$entry->trackNo->value])) {
                return new Err(new DomainValidationError([
                    'trackEntries' => ['同じ曲順を複数指定することはできません。'],
                ]));
            }

            $seenSongIds[$entry->songId->value] = true;
            $seenTrackNos[$entry->trackNo->value] = true;
            $entries[] = $entry;
        }

        return new Ok(new self($entries));
    }
