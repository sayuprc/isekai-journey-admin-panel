...
readonly class UpdateInteractor implements UpdateUseCaseInterface
{
    ...
    public function handle(UpdateInputData $inputData): Result
    {
        ...
        return SongTagId::create($inputData->songTagId)
            ...
            ->andThen(function (SongTagId $songTagId) use ($inputData): Result {
                if (is_null($this->repository->find($songTagId))) {
                    return new Err(new NotFoundError('楽曲タグ', $songTagId->value));
                }

                return $this->transaction->scope(function () use ($songTagId, $inputData): Result {
                    $result = $this->service->prepareForUpdate($songTagId->value, $inputData->name, $inputData->orderNo);
